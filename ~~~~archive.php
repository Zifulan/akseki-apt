...
if(!empty($curcatanc)){
	if($current_category_id == $catid_summit){
		...
		$catanc = "ministerial";
	}elseif($current_category_id == $catid_cpr or $current_category == "aptwg"){
		$catanc = "cpr";
...
if(($catanc == "ministerial" or $catanc == "som" or $catanc == "othlev") and !empty($curcatanc)){
...
/*function echo_category_descriptor($current_category_id){
	...
	foreach($catdesc_children as $child):
		...
		if(get_queried_object()->name === $child->post_title):
	...
	<div class="container" id="avi-content">		
		...
		<div class="listings-wrapper row">		
			...
			<?php					
				...
				//the content
				if(($catlev == "level") and ($catanc == "ministerial" or $catanc == "som" or $catanc == "othlev")){					
					if(!empty($curcat_children)){
						//if list of children categories, use this
						//pillars bar
						echo('<ul class="nav nav-pills w-100 mb-3 nav-justified" id="commision-pills-tab">');
						foreach($curcat_children as $key=>$child) :
							...
							echo '
							<li class="nav-item '.($key!==0?"ml-2":"").'">
								<button class="nav-link btn btn-outline-primary w-100'.$active.'" id="commision-'.$child->slug.'-tab" data-toggle="pill" href="#commision-'.$child->slug.'" role="tab" aria-controls="commision-'.$child->slug.'" aria-selected="true">'.$child->name.'</button>
							</li>';						
						endforeach;
						echo('</ul>');
					}
					...
					echo('<div class="tab-content order-4 col-12" id="commisionTabContent">');
					...
					if( $cl->have_posts() ) {
						...
						while ( $cl->have_posts() ):
							...
							switch($catancgroup){
								case "singlelevel":
									if($current_category == 'cpr-level' or $current_category == 'aptwg'){
										...
										if ( has_tag("marker") ) {
											if(get_post_status($theid) == 'future'){
												$newgroups[$year_mon]["markerimage"] = wp_get_attachment_image(1007, 'medium', '', array('class' => 'rounded img-fluid float-left w-100'));
											}else{
												$newgroups[$year_mon]["markerimage"] = get_the_post_thumbnail( $theid, 'medium', ['class' => 'rounded img-fluid float-left w-100'] );
											}
										...
										/*$markerimage = '<div class="rounded" style="height:400px;">';
										$markerimage .= get_the_post_thumbnail( $theid, 'large', ['class' => 'img-fluid '.$theid] );
										$markerimage .= '</div>';
										$inner .= '<div class="tab-pane fade '.$show.$active.'" id="v-pills-'.$year_mon.'" role="tabpanel" aria-labelledby="v-pills-'.$year_mon.'-tab">';
										
										$inner .= $markerimage;
										$inner .= '<ul class="list-group list-group-flush">';
										// Display post title.
										if ( $title = get_the_title() ) {
											$inner .= "<li class='list-group-item'><a href='".get_permalink()."'>".$title."</a></li>";
										} else {
											$inner .= "<li class='list-group-item'>".$current_category_id."</li>";
										}
										//get posts in the summit category published in the same month and year
										$tagmarker = get_term_by('name', 'marker', 'post_tag');
										$ssqc_args = array(
										  'category' => $current_category_id,
										  'm' => $year_mon,
										  'tag__not_in' => array($tagmarker->term_id)
										);

										$ssqc = get_posts( $ssqc_args );
										if(!empty($ssqc)){
											foreach($ssqc as $eachssqc){
												$inner .= '<li class="list-group-item"><a href="'.get_permalink($eachssqc->ID).'">'.$eachssqc->post_title.'</a></li>';
											}
										}
										...
										$inner .= "</div>";*/
										//buttons
										//$buttons .= ('<a type="button" class="btn btn-light m-1 nav-link'.$active.'" id="v-pills-'.$year_mon.'-tab" data-toggle="pill" href="#v-pills-'.$year_mon.'" role="tab" aria-controls="v-pills-'.$year_mon.'" aria-selected="true">'.$year.'</a>');
						...
						if($catanc == "summit"){
							...
							//echo($inner);
							//echo('</div>');//end of tabContent
							//flex for two columns
							/*echo('<style>
							.listings-wrapper .nav {
							  display: flex;
							  flex-wrap: wrap;
							  margin: 0;
							  box-sizing: border-box;
							}
							.listings-wrapper .nav-link {
							  flex: 0 0 45%;
							  padding: 10px;
							}
							</style>');*/
		...
		?>
			...
			?>
			<div class="<?php echo($sidebarcol); ?> <?php echo($extraclass); ?>-sidebar order-3">
				<?php
				//image logo
				if($catlev == "topic" or $catanc == "cpr"){
					//echo('<img src="https://koran-jakarta.com/images/article/phpuycikq_resized.png" class="w-100 mb-2">');3347
				...
				if($catlev == "pillar"){
					...
					foreach($sidelists as $sidelist){
						echo('<h5 class="mt-4">Cooperation '.$sidelist.'s</h5>');
						if($sidelist == "Level"){
							$qterm = 7;
						...
						foreach($cs as $child){
							echo('<a title="'.$child->description.'" class="pillar-list-link list-group-item list-group-item-action list-group-item-secondary" href="'.get_term_link($child->term_id).'">'.$child->name.'</a>');
						}
						echo('</div>');
					}
					//styling
				...
				if($catlev == "topic" and ($catanc == 'ministerial' or $catanc == 'som' or $catanc == "othlev")):
					$minsomtit = "";
					$minsomanc = -1;
					...
					echo('</ul>');
					echo('<h3 class="w-100 btn btn-primary" style="margin-top:3em;"><i class="fas fa-arrow-right mr-2"></i>Statements</h3>');
					echo('<ul class="list-group list-group-flush">');
					$wlc2_args = array(
						'echo' => true,
						'depth'               => 2,
