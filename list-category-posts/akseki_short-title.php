...
$future_post = akseki_get_future_post_in_category($this->params['id']);
if(!empty($future_post)){
	foreach($future_post as $fpid){
		$short = akseki_get_shortname(get_the_title($fpid),$fpid);
		$lcp_display_output .= '<li class="list-group-item p-1"><em style="color:gray;font-size:.9em;">'.preg_replace('/numberer/','',$short).' (upcoming)</em>';
		$sc = akseki_get_sc($fpid);
		if(empty($sc)){
			$lcp_display_output .= ' <span class="badge badge-secondary"><em>&nbsp;tbd.&nbsp;</em></span>';
		}else{
			$lcp_display_output .= ' <span class="badge badge-secondary">' . substr(strip_tags($sc),-4) . '</span>';
		}
		$lcp_display_output .= ' <span class="roundflag">'.akseki_get_the_flag($fpid).'</span></li>';
	}
}
while ( have_posts() ):
	the_post();
	...
	if($noshorttitle){
		...
		//$lcp_display_output .= preg_replace('/(\d\d*)(st|nd|rd|th)/', '$1<sup>$2</sup> APT', $fixedtitle);
		$lcp_display_output .= preg_replace('/(\d\d*)(st|nd|rd|th)/', '$1<sup>$2</sup> ', $fixedtitle);
	}
	if(empty($SM)){
		$lcp_display_output .= ' '.$this->get_category_link('span');
	}
	if($current_post_ID !== get_the_ID()){
